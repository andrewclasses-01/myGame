// ⭐⭐⭐⭐⭐ MẪU 6d (thầy 27/9/2026): thanh MISS WAIT 3D giữa màn dưới câu hỏi (./rr3d-misswait.js, view.setMissWait) · tên lửa nạp BẰNG TAY,
// vết cháy + lửa nhỏ trên thân tàu bị trúng, chuông báo động mới (rr3d-missile.js). Chỗ sửa đánh dấu "mẫu 6d".
// ⭐⭐⭐⭐ MẪU 6c (thầy 27/9/2026): bỏ khung ô tên lửa + BOOST, né LÙI khi sai bị trừ điểm (rr3d-missile.js). View không đổi.
...
import { createMissWait } from "./rr3d-misswait.js";   // mẫu 6d
...
export async function createView(cfg) {
  ...
  function buildUI() {
    ...
    paintQuestion();
    MW.build(U, qRect.y + qRect.h);            // mẫu 6d: thanh MISS WAIT ngay dưới câu hỏi
    // 2 bàn đáp án
  ...
  const MS = cfg.missiles === false ? null : createMissiles({
    ...
    onLoad: side => cfg.onLoad && cfg.onLoad(side)                 // mẫu 6d: chạm quả nhỏ = nạp
  });
  const MW = createMissWait({ THREE, ui, screenToLocal, screenSize, frameGeo, canvasTex, radialTex, FONT_UI, TEAMS, G, UID });   // mẫu 6d
  warmBoom();
  ...
  function tick(dt) {
    ...
    MW.tick(dt);                               // mẫu 6d: thanh MISS WAIT
  ...
  return {
    ...
    rockets, camera,
    setMissWait(v) { MW.set(v); },            // mẫu 6d: { side, frac 0..1, secs } | null — thanh MISS WAIT giữa màn
