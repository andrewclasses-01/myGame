// MẪU 6b (26/9/2026): camera({ wide }) — bắn tên lửa ⇒ góc RỘNG, chuyển nhanh 1,1 s.
...
export function RR3D_CFG(V) {
  ...
  return {
    ...
    camera({ t, trail, rockets, L, wide }) {
      ...
      // mẫu 6b: bắn tên lửa ⇒ góc RỘNG (chuyển nhanh 1,1 s để kịp thấy quả vòng lên + lao xuống)
      ...
      // mẫu 6b: góc RỘNG của tên lửa — vừa khít 2 tàu + đỉnh vòng bay, nhìn chéo từ trên cao phía sau-bên (2 làn tách rõ trên màn)
      ...
      if (wideK > 0.001) {
        ...
        const D = 15 + span * 0.85 + Hm * 0.7;
        const look = P(0, Hm * 0.32, zm - 1), dir = P(0.62, 0.55, 0.56).normalize();
        const wpos = look.clone().addScaledVector(dir, D).add(P(Math.sin(t * 0.15) * 0.6, 0, 0));
