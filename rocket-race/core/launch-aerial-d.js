// =============================================================
// MẪU 4d (thầy 26/9/2026: "bản đồ thiết kế lại để THỰC SỰ giống ảnh" nhà xưởng SpaceX ở 39A): dựng lại TOÀN BỘ bố cục
...
export async function createLaunch(cfg) {
  ...
  // ----- CÂY: rừng bụi Florida 3D (chỉ ngoài đồng cỏ, không trên nước) — tạo chiều sâu tiền cảnh -----
  {
    ...
    const N = 9000, im = new THREE.InstancedMesh(geo, new THREE.MeshStandardMaterial({ color: "#ffffff", roughness: 1 }), N);
    const m4 = new THREE.Matrix4(), q = new THREE.Quaternion(), col = new THREE.Color();
    let n = 0;
    for (let i = 0; i < N * 6 && n < N; i++) {
      const x = rand(-230, 120), z = rand(-160, 190);
      if (inField(x, z) || segD(x, z) < 20 || inLake(x, z) || coast(x, z) > -12) continue;
      const s = rand(0.5, 1.2);
      m4.compose(new V3(x, s * 0.3, z), q.setFromAxisAngle(new V3(0, 1, 0), rand(0, TAU)), new V3(s * rand(1, 1.5), s * rand(0.45, 0.7), s * rand(1, 1.5)));
      im.setMatrixAt(n, m4); im.setColorAt(n, col.setHSL(rand(0.2, 0.3), rand(0.4, 0.6), rand(0.04, 0.085))); n++;
    }
