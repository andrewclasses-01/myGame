// =============================================================
// MẪU 4g (thầy 26/9/2026): logo + chữ ANDREW STUDIO trên nhà xưởng "chưa đẹp và cân đối" ⇒ HUY HIỆU mới (tròn, tên lửa, quỹ đạo)
...
// 4g: HUY HIỆU ANDREW STUDIO — tròn đặc màu mực, viền trắng mảnh, tên lửa trắng chếch 45° có lửa, vòng quỹ đạo quấn quanh
function drawEmblem(g, cx, cy, R, ink) {
  g.save(); g.translate(cx, cy);
  g.fillStyle = ink; g.beginPath(); g.arc(0, 0, R, 0, TAU); g.fill();
  g.strokeStyle = "#ffffff"; g.lineWidth = R * 0.045; g.beginPath(); g.arc(0, 0, R * 0.87, 0, TAU); g.stroke();
  const orbit = (a0, a1) => { g.save(); g.rotate(-0.38); g.strokeStyle = "rgba(255,255,255,.92)"; g.lineWidth = R * 0.055; g.lineCap = "round";
    g.beginPath(); g.ellipse(0, R * 0.04, R * 0.68, R * 0.22, 0, a0, a1); g.stroke(); g.restore(); };
  orbit(Math.PI, TAU);                                                   // nửa sau quỹ đạo (sau tên lửa)
  g.save(); g.rotate(Math.PI / 4); const s = R;
  const fl = g.createLinearGradient(0, s * 0.28, 0, s * 0.8); fl.addColorStop(0, "#ffd23a"); fl.addColorStop(0.45, "#ff7a1a"); fl.addColorStop(1, "rgba(230,40,30,0)");
  g.fillStyle = fl; g.beginPath(); g.moveTo(-s * 0.085, s * 0.3); g.quadraticCurveTo(0, s * 0.88, s * 0.085, s * 0.3); g.closePath(); g.fill();
  g.fillStyle = "#e3342f"; [-1, 1].forEach(k => { g.beginPath(); g.moveTo(k * s * 0.1, s * 0.04); g.lineTo(k * s * 0.25, s * 0.33); g.lineTo(k * s * 0.1, s * 0.27); g.closePath(); g.fill(); });
  g.fillStyle = "#ffffff"; g.beginPath(); g.moveTo(0, -s * 0.54);
  g.bezierCurveTo(s * 0.17, -s * 0.36, s * 0.13, s * 0.1, s * 0.1, s * 0.3); g.lineTo(-s * 0.1, s * 0.3); g.bezierCurveTo(-s * 0.13, s * 0.1, -s * 0.17, -s * 0.36, 0, -s * 0.54); g.fill();
  g.fillStyle = "#e3342f"; g.beginPath(); g.moveTo(0, -s * 0.54); g.bezierCurveTo(s * 0.08, -s * 0.46, s * 0.11, -s * 0.4, s * 0.125, -s * 0.36); g.lineTo(-s * 0.125, -s * 0.36); g.bezierCurveTo(-s * 0.11, -s * 0.4, -s * 0.08, -s * 0.46, 0, -s * 0.54); g.fill();   // mũi đỏ
  g.fillStyle = ink; g.beginPath(); g.arc(0, -s * 0.13, s * 0.07, 0, TAU); g.fill(); g.strokeStyle = "#e3342f"; g.lineWidth = s * 0.025; g.stroke();   // cửa sổ
  g.restore();
  orbit(0, Math.PI);                                                     // nửa trước quỹ đạo (đè lên tên lửa)
  g.restore();
}
// cụm HUY HIỆU + "ANDREW STUDIO" căn giữa tại (cx, cy); tự thu cỡ cho vừa bề rộng maxW; trả về bề rộng cụm
function drawLockup(g, cx, cy, capH, ink, maxW = Infinity) {
  const fit = h => { g.font = `italic 900 ${h / 0.7}px "Exo 2", "Bahnschrift", "Arial Black", sans-serif`; if (g.letterSpacing !== undefined) g.letterSpacing = `${h * 0.16}px`;
    const tw = g.measureText("ANDREW STUDIO").width, Rr = h * 1.25, gap = h * 0.6; return { tw, Rr, gap, total: Rr * 2 + gap + tw }; };
  let m = fit(capH); if (m.total > maxW) { capH *= maxW / m.total; m = fit(capH); }
  const x0 = cx - m.total / 2;
  drawEmblem(g, x0 + m.Rr, cy, m.Rr, ink);
  g.fillStyle = ink; g.textAlign = "left"; g.textBaseline = "alphabetic";
  const tx = x0 + m.Rr * 2 + m.gap, base = cy + capH * 0.36;
  g.fillText("ANDREW STUDIO", tx, base);
  g.fillStyle = "#d9302b"; g.fillRect(tx + capH * 0.08, base + capH * 0.26, m.tw - capH * 0.3, capH * 0.07);   // gạch chân đỏ mảnh
  return m.total;
}
function signTex() {
...
export async function createLaunch(cfg) {
  ...
  const MD = LAY.mound, PY = MD.h;
  {
    ...
    const topT = (() => {                                   // mặt sân: tấm bê tông sáng, ron, vết cháy quanh 2 bệ, rãnh thoát lửa
      ...
      drawLockup(gg, W / 2, (7.4 - MD.z0) * sx, sx * 2.5, "#1c2c5a", W * 0.86);
      gg.restore();
      return tex(c); })();
  ...
  // ----- NHÀ XƯỞNG LỚN (như ảnh) -----
  {
    ...
    const facade = panelWall(W, H, (c, Wp, Hp) => {    // mặt dài quay về máy quay: logo tròn (chỗ lá cờ) + ANDREW STUDIO chữ nghiêng xanh đậm
      ...
      drawLockup(c, Wp * 0.5, Hp * 0.31, Hp * 0.19, "#1c2c5a", Wp * 0.74);
