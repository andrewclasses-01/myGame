// =============================================================
// ROCKET RACE 3D — TÊN LỬA TẤN CÔNG giữa 2 tàu (MẪU 6b, thầy 26/9/2026 — sửa từ mẫu 6)
...
export function createMissiles(X) {
  ...
  const MC = Object.assign({ dur: 3.8, window: 1.5, ammoCm: 8.5, boostCm: 3.2, gapCm: 0.9 }, cfg.missiles || {});
  ...
  // ---------------------------------------------------------------- 6b: MỘT ô tên lửa + thanh BOOST (không chữ)
  function box(w, h, team, depth = 0.06) {
    const g = new THREE.Group();
    const rr = Math.min(w, h) * 0.18;
    const slab = new THREE.Mesh(new RoundedBoxGeometry(w, h, depth, 4, rr),
      new THREE.MeshPhysicalMaterial({ color: new THREE.Color("#070b16"), metalness: 0.7, roughness: 0.45, envMapIntensity: 0.2 }));   /* ĐẶC: vệt bụi không vẽ đè */
    ...
    const rim = new THREE.Mesh(frameGeo(w + 0.03, h + 0.03, rr * 1.05, Math.min(w, h) * 0.05), rimMat); rim.position.z = depth / 2 + 0.005;
    g.add(slab, rim);
    return { g, slab, rim, rimMat, w, h };
  }
  const fat = (g, k) => g.scale.set(k, k * 1.35, k * 1.35);       // quả trong ô: mập hơn cho dễ nhìn
  function buildConsole(con, inner, s, pad, cm) {
    ...
    const am = box(areaW, ammoH, team); am.g.position.set(0, yAmmo, 0.05); inner.add(am.g);
    const bigW = areaW * 0.64, spW = areaW * 0.28;
    const bigX = inSign * (areaW / 2 - bigW / 2 - areaW * 0.03), spX = -inSign * (areaW / 2 - spW / 2 - areaW * 0.04);
    const miniS = spW * 0.9 / 2.3, bigS = bigW * 0.92 / 2.3;
    const rotY = inSign > 0 ? 0 : Math.PI;                    // mũi chĩa về phía tàu địch
    const minis = [0.2, -0.2].map(fy => { const m = makeMissile(); m.g.position.set(spX, fy * ammoH, 0.14); m.g.rotation.y = rotY; m.g.visible = false; am.g.add(m.g); return m; });
    const big = makeMissile(); big.g.position.set(bigX, 0, 0.18); big.g.rotation.y = rotY; big.g.visible = false; am.g.add(big.g);
    ...
    glowSp.scale.set(bigW * 1.25, ammoH * 1.25, 1); glowSp.position.set(bigX, 0, 0.16); am.g.add(glowSp);
    const shuttle = makeMissile(); shuttle.g.visible = false; shuttle.g.rotation.y = rotY; am.g.add(shuttle.g);
    // thanh BOOST: khung + dải CYAN dài theo năng lượng (gốc ở mép NGOÀI, dài vào giữa màn)
    const bst = box(areaW, boostH, team, 0.05); bst.g.position.set(0, yBoost, 0.05); inner.add(bst.g);
    const innerW = areaW - boostH * 0.4, fillH = boostH * 0.52;
    const track = new THREE.Mesh(new THREE.PlaneGeometry(innerW, fillH), new THREE.MeshBasicMaterial({ color: new THREE.Color("#0f1a26"), transparent: true, opacity: 0.95, depthWrite: false }));
    track.position.z = 0.032; bst.g.add(track);
    const fillGeo = new THREE.PlaneGeometry(1, fillH); fillGeo.translate(0.5, 0, 0);
    const fillMat = new THREE.MeshBasicMaterial({ color: new THREE.Color(0.4, 2.4, 3), transparent: true, opacity: 1, depthWrite: false });
    const fill = new THREE.Mesh(fillGeo, fillMat);
    fill.position.set(-inSign * innerW / 2, 0, 0.036); fill.scale.x = 0.0001 * inSign; bst.g.add(fill);
    // khung cảnh báo đỏ bao cả cột (đáp án + ô tên lửa + BOOST)
    ...
    [[am.slab, "fire"], [bst.slab, "boost"]].forEach(([m, kind]) => { m.userData.ammo = { side, kind }; hitList.push(m); });
    UI[side] = { am, bst, minis, big, glowSp, shuttle, fill, fillMat, innerW, warn, warnMat, inSign, miniS, bigS, bigX, spX, ammoH,
      press: { fire: 0, boost: 0 }, pop: [0, 0], popBig: 0, load: null, fireK: 0, shakeK: 0, flash: 0, fillK: 0 };
  }
  function tickUI(side, dt) {
    ...
    if (U.fireK > 0) {                     // bắn: quả to vút về phía giữa màn rồi mất
      ...
      U.big.g.position.x = U.bigX + U.inSign * k * k * U.am.w * 0.9; fat(U.big.g, U.bigS * (1 + k * 0.4));
      ...
      U.big.g.position.y = Math.sin(G.t * 2.2) * 0.01;
    }
    ...
    U.am.rimMat.color.copy(ready ? new THREE.Color(4, 0.6, 0.45).multiplyScalar(0.8 + 0.25 * Math.sin(G.t * 5)) : tc.clone().multiplyScalar(a.on ? 1.2 : 0.3));
    // thanh BOOST: dải cyan dài theo số câu liên tiếp (đầy = có BOOST), trượt mượt
    const want = a.boost ? 1 : clamp(a.boostPips / a.boostMax, 0, 1);
    U.fillK += (want - U.fillK) * Math.min(1, dt * 6);
    U.fill.scale.x = Math.max(0.0001, U.fillK * U.innerW) * U.inSign;
    const pul = 0.5 + 0.5 * Math.sin(G.t * (win ? 16 : 5));
    const full = a.boost && !a.locked;
    // màu CYAN rõ (không đẩy sáng quá mức ⇒ bloom không làm trắng dải); đầy = sáng hơn + đập nhịp, lúc né được thì đập nhanh
    U.fillMat.color.setRGB(0, 0.55, 0.78).multiplyScalar(full ? (win ? 1.1 + 1.0 * pul : 1.25 + 0.2 * pul) : 1);   /* ACES làm nhạt màu sáng ⇒ giữ cường độ thấp cho ra CYAN đậm */
    U.bst.rimMat.color.copy(full && win ? new THREE.Color(0.8, 3.2, 4).multiplyScalar(0.7 + 0.6 * pul)
      : inc < Infinity ? new THREE.Color(4, 0.4, 0.35).multiplyScalar(0.5 + 0.6 * pul)
      : full ? new THREE.Color(0.5, 2.2, 2.8) : tc.clone().multiplyScalar(0.45));
    ["fire", "boost"].forEach(k => { U.press[k] = Math.max(0, U.press[k] - dt * 4); });
    U.am.g.scale.setScalar(1 - U.press.fire * 0.05);
  ...
  function dodge(side) {
    ...
    if (any) {
      ...
      burst(shipPos(r).addScaledVector(TRAVEL, -2.6), { n: 90, speed: 9, color: new THREE.Color(1.2, 2.6, 5), colorEnd: new THREE.Color(0.2, 0.4, 1.6), size: 0.24, life: 0.6 });
    }
    return any;
  }
  ...
  function poseRocket(r, i, dt) {
    ...
    r.rig.position.addScaledVector(TRAVEL, 3.6 * k);
